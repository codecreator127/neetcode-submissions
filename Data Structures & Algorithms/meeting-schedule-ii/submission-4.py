...
class Solution:
    def minMeetingRooms(self, intervals: List[Interval]) -> int:
        ...
        for i in range(len(intervals)):
            ...
            ## check overlap
            if heap and curr.start < heap[0][0]:
                heapq.heappush(heap, (curr.end, curr.start))
            else:
                ## no overlap, then we can evict all rooms that have ended
                while heap and heap[0][0] <= curr.start:
                    heapq.heappop(heap)
                heapq.heappush(heap, (curr.end, curr.start))
